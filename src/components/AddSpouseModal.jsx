...
const AddSpouseModal = ({ isOpen, onClose, onAddSpouse, member }) => {
  const [formData, setFormData] = useState({ name: '', gender: '', birthYear: '', job: '', phone: '', email: '', address: '', notes: '' });
  ...
  const handleSubmit = (e) => {
    ...
    const newSpouse = {
      // Use a consistent spouse id suffix '-s' so UI detects dâu/rể via `id.endsWith('-s')`
      id: `${member.id}-s`,
    ...
    onAddSpouse(member.id, newSpouse);
    console.log('[AddSpouseModal] submitted newSpouse for member:', member?.id, newSpouse);
    handleClose();
  };
    ...
    <div onClick={handleClose} style={{ position: 'fixed', inset: 0, background: 'rgba(17,24,39,0.5)', backdropFilter: 'blur(2px)', display: 'grid', placeItems: 'center', zIndex: 1000 }}>
      <div onClick={e => e.stopPropagation()} style={{ width: 600, maxWidth: '96vw', maxHeight: '86vh', overflow: 'hidden', display: 'flex', flexDirection: 'column', background: '#fff', borderRadius: 16, boxShadow: '0 20px 60px rgba(0,0,0,0.25)', border: '1px solid #e5e7eb', animation: 'modalIn 0.18s ease-out' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: 20, borderBottom: '1px solid #f3f4f6' }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 20, fontWeight: 700, color: '#111827' }}>Thêm vợ/chồng mới</div>
            <div style={{ fontSize: 12, color: '#6b7280' }}>Thêm vợ/chồng cho {member?.name}</div>
          </div>
            ...
            <div style={{ display: 'grid', gap: 14 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                ...
                <div>
                  ...
                  <div className={`field ${errors.birthYear ? 'error' : ''}`}>
              ...
              <div>
                ...
                <div className={`field ${errors.job ? 'error' : ''}`}>
              ...
              <div style={{ display: 'flex', gap: 12, marginTop: 6, paddingTop: 12, borderTop: '1px solid #f3f4f6' }}>
                ...
                <Button type="submit" className="btn-confirm">
                  <i className="bi-plus-lg" style={{ color: '#fff' }} /> Thêm vợ/chồng
                </Button>
              </div>
            </div>
          </form>
